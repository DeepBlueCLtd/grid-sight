/**
 * Parse helpers for the `gs.v` URL fragment. Kept in a separate module so
 * `view-state-decode.ts` exposes only the public entry point and Codacy's
 * per-file complexity metric stays bounded.
 *
...
export function parseSegment(seg: string): TableViewDirective | null {
