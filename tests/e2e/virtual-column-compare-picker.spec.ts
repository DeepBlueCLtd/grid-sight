 ...
 * the real lozenge UI — the Δ lozenge pulses while armed, each picked column
 * header pulses, pulses clear once the column appears, and clicking off a
...
test.describe('Virtual columns — compare picker affordance', () => {
  ...
  test('Δ pulses, picked columns pulse, then clear when the column shows', async ({ page }) => {
    await enable(page);

    const delta = page.locator('#sales-table .gs-vc-lozenge[data-gs-vc-kind="compare"]');
    await delta.click();

    // Δ pulses + numeric headers become candidates.
    await expect(delta).toHaveClass(/gs-vc-pulse/);
    ...
    // Pick Q1 (header index 1) — it pulses.
    const q1 = page.locator('#sales-table thead th').nth(1);
    await q1.click();
    await expect(q1).toHaveClass(/gs-vc-pulse/);

    // Pick Q4 (header index 4) — compare column appears, pulses clear.
    ...
    await expect(delta).not.toHaveClass(/gs-vc-pulse/);
    expect(await page.locator('#sales-table .gs-vc-pulse').count()).toBe(0);
