...
|-----------|---------|-------|
| I. Lightweight & Minimal Dependencies | ⚠️ Conditional Pass | **No new runtime dependency.** Net IIFE delta budgeted **≤ 1.5 KB gzipped**, measured by `scripts/bundle-size.js`. The bundle already exceeds the §I 10 KB ceiling (~19 KB) under a pre-existing, explicitly-recorded violation with an enforced 25 KB interim cap; this feature inherits that posture and must stay under 25 KB. It does **not** introduce a *new* violation and does not raise the cap. Tracked in Complexity Tracking. |
...
|-----------|------------|-------------------------------------|
| Bundle exceeds §I 10 KB gzipped ceiling (inherited; enforced 25 KB interim cap) | The bundle was already ~19 KB before this feature (recorded override in `specs/012-capability-filtering/baseline-bundle-size.md`). This feature adds ≤ 1.5 KB and cannot land under a 10 KB gate that the tree already violates. | Shipping copy-as-CSV under a hard 10 KB gate would require first landing an unrelated bundle-cut PR or a constitution budget-raise amendment — both out of scope here. The interim 25 KB cap (already enforced by `scripts/bundle-size.js`) bounds this feature's growth; the formal 10 KB resolution remains the separate amendment PR called out in the baseline doc. |
